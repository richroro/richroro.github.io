...
> **자체 도메인을 쓰면** `index.html` 맨 위 CSP 의 `connect-src` 에 그 주소를 더한다.
> 기본은 `https://*.supabase.co` 만 열려 있어서, 다른 주소로는 브라우저가 연결 자체를 막는다.
...
## 3. 개인정보처리방침과 이용약관 채우기 (15분)

`privacy.html` 과 `terms.html`(이용약관 + 운영정책)은 앱이 실제로 하는 일에 맞춰 쓴 **초안**이다.
운영자만 아는 사실이 노란 칸으로 비어 있다.
...
`terms.html`
1. 운영자 이름 (맨 위, 제2조)
2. 권리침해 신고와 이의 신청을 받는 메일 주소 (운영정책 3조·5조)
3. 관할 법원 (제15조), 시행일 (부칙)

다 채웠으면 두 문서 맨 위의 `<div class="draft" id="draftNote">` 상자를 지운다.
게시 전에 전문가의 검토를 받는다. 이 문서들은 법률 자문이 아니다.

약관의 숫자(신고 몇 명이면 가림, 도배 한도, 임시조치 30일, 처리 기록 1년)는 `test/terms.mjs` 가 서버 SQL 과 대조한다.
SQL 의 숫자를 바꾸면 약관도 같이 고쳐야 검사가 통과한다.
...
`server/test-sql.mjs` 에 계정 삭제 쪽 검사가 함께 있다. 준호가 탈퇴하면 계정·프로필·리포트·신고가 다 사라지고,
...
| 공용 보드로 받아 간 이웃 기기 | **다음 전체 대조 때 빠진다** — 앱을 열 때, 30분마다, 손으로 받아올 때. 받아온 범위 안에서 서버에 없는 글을 이 기기에서도 뺀다 |
| 링크로 받아 간 대화방·기기 | **회수할 수 없다.** 링크 안에 내용이 통째로 실려 있다 |

마지막 줄은 기술로 해결되지 않는다. 그래서 삭제 화면과 처리방침 7조에 그대로 적었다.

같은 규칙이 신고에도 적용된다. 세 사람이 신고해서 가려진 글은 이미 받아 간 이웃 기기에서도 다음 전체 대조 때 빠진다.
(평소 3분마다 받아올 때는 새 글만 묻는다 — 주고받는 양이 수십 분의 일로 준다. README 의 "받아오기는 가볍게")
...
- **정보통신망법 게시판 의무** — 신고 창구(신고 단추), 권리침해 신고와 임시조치(운영정책 3조, 운영 화면),
  불법정보 삭제, 운영자 연락처 표시(방침 14조).
- **이용약관** — `terms.html` 초안이 있다. 로그인 확인란이 약관 동의를 겸한다(만 14세 확인과 같은 칸).
- 수익이 생기면 사업자등록.
