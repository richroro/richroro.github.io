...
- 사용자 식별키: 현재는 로컬 익명 ID. 토스에선 게임센터 프로필로 대체
- 리더보드: `Game.setLeaderboardScore` / `Game.openLeaderboard`
- 미니앱 종료: `close()`

이 네 가지는 `index.html` 상단의 **`Toss` 어댑터** 한 곳에 모아 두었다.
토스 웹뷰가 브릿지를 주입하면 자동으로 그걸 쓰고, 없으면 일반 웹으로 똑같이 동작한다.

## 앱인토스에 실제로 올리려면

SDK(`@apps-in-toss/web-framework`)는 번들러가 필요해서 지금의 무빌드 정적 파일에는
직접 넣을 수 없다. 콘솔 등록까지 마친 뒤 아래 순서로 옮기면 된다.

1. [사업자 등록](https://developers-apps-in-toss.toss.im/guide/operation/register-business.md) — 개인사업자 등록증 필요
2. 콘솔에서 미니앱 등록 (앱 로고, 앱 이름, `appName`, 사용 연령, 고객센터 연락처, 게임 카테고리)
3. 프로젝트 생성 후 이 `index.html`의 게임 로직을 그대로 옮긴다

   ```bash
   npx create-ait-app tikkeul-moa
   ```

4. `Toss` 어댑터의 네 함수만 실제 SDK 호출로 교체

   ```js
   import { Game, close } from '@apps-in-toss/web-framework';
   // submitScore -> Game.setLeaderboardScore({ score })
   // openBoard   -> Game.openLeaderboard()
   // userKey     -> Game.getUserProfile()
   // exitApp     -> close()
   ```
