/* 공모주 캘린더 서비스 워커 — 오프라인에서도 마지막으로 본 일정으로 열리게 합니다.
   ...
   실패할 때만 캐시를 씁니다. */
const CACHE = "ipo-v1";
const SHELL = ["/ipo/", "/ipo/app.js", "/ipo/ipo.css", "/m7/assets/m7.css", "/ipo/icon.svg", "/ipo/manifest.webmanifest"];
...
self.addEventListener("fetch", (e) => {
  ...
  e.respondWith(fetch(req).then((res) => {
    ...
    return res;
  }).catch(() => caches.match(req, { ignoreSearch: url.pathname.endsWith("/") }).then((hit) => hit || caches.match("/ipo/"))));
});
