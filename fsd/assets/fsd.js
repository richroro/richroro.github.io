...
/* 인식 층 — 켜고 끌 수 있는 단위 */
export const LAYERS = {
  lane: { ko: '차선', desc: '카메라가 본 차선을 이어 붙인 선. 지도가 아니라 지금 보이는 것으로 그린다.', color: 0x6fe3ff },
  object: { ko: '물체', desc: '차·사람·표지처럼 움직이거나 부딪힐 수 있는 것. 상자로 감싸고 무엇인지, 얼마나 확신하는지를 붙인다.', color: 0x7cf2b0 },
  drivable: { ko: '주행 가능 영역', desc: '갈 수 있다고 판단한 바닥. 차선 안쪽만이 아니라 갓길·교차로까지 포함한다.', color: 0x4b8fe8 },
  path: { ko: '계획 경로', desc: '앞으로 몇 초 동안 갈 길. 끼어드는 차가 생기면 이 리본이 먼저 휜다.', color: 0xffc86b },
  occupancy: { ko: '점유 격자', desc: '공간을 작은 칸으로 쪼개고 칸마다 무언가 있는지를 표시한다. 상자로 못 감싸는 것(난간·연석·잔해)까지 담는다.', color: 0xb98cff },
};
...
function buildCar(bodyMat, glassMat, wheelMat, scale = 1) {
  ...
  const body = new THREE.Mesh(extrudeSide(CAR_PROFILE, 1.86), bodyMat);
  g.add(body);
  const cabin = new THREE.Mesh(extrudeSide(CABIN_PROFILE, 1.72, 0.04), glassMat);
  cabin.position.y = 0.01;
  g.add(cabin);
  const wheelGeo = new THREE.CylinderGeometry(0.35, 0.35, 0.26, 18).rotateZ(Math.PI / 2);
  for (const sx of [-1, 1]) for (const sz of [1.42, -1.48]) {
...
export function createFSD(canvas, opts = {}) {
  ...
  scene.add(key);
  const rim = new THREE.DirectionalLight(0x86c8ff, 1.0); rim.position.set(9, 4, -12);
  scene.add(rim);
  ...
  const roadMat = new THREE.MeshPhysicalMaterial({
    map: roadTex, roughness: 0.82, metalness: 0.05, roughnessMap: rough,
    envMapIntensity: 0.55, transparent: true, opacity: 1,
  });
  ...
  const RIGS = {
    chase: { pos: (x) => V3(x * 0.4, 4.5, -10.8), look: (x) => V3(x * 0.85, 1.15, 17) },
  ...
  function frame() {
    ...
    renderer.info.reset();
    composer.render(scene, camera, { bloom: 0.62, threshold: 1.12, exposure: PALETTE.exposure, grain: 0.013, time: t });
  }
