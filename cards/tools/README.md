...
테마별로 `ink`·`muted`·`faint`·`accent`가 얹히는 면과 이루는 명도 대비를
WCAG 식으로 계산한다. 팔레트를 건드린 뒤에는 이걸 돌린다.

눈으로만 맞추다가 실제로 틀린 적이 있다. `neon`의 강조색이 13:1로 본문
글자(18:1)와 맞먹어서 눈이 부셨고, 정작 작은 글씨는 3.7에 머물렀다.
대비가 필요한 곳과 들어간 곳이 어긋나 있었던 것이다.

기준은 이렇게 잡는다. 본문·작은 글씨는 4.5 이상, 강조색은 4.5~9 사이.
강조색이 본문보다 밝아지면 강조가 아니라 광원이 된다.
...
제목·설명·태그를 콘텐츠 JSON에서 만들어낸다. 손으로 쓰지 않는 이유는 두 가지다.
카드를 고쳤는데 설명이 옛날 숫자를 말하는 일이 없고, 하단 고지 앞 세 줄이
설명에 자동으로 들어가 **출처와 기준일이 영상 밖에도 남는다.** 시세처럼
움직이는 숫자는 기준이 빠지면 주장이 된다.

제목과 태그는 JSON의 `youtube` 항목에 둔다.

```json
"youtube": { "title": "전국에서 가장 비싼 아파트 1위, 325억",
             "tags": ["부동산","아파트","공시가격"] }
```
...
- 배경 그라데이션 없이 단색
- 제목 블록 아래 **64px**을 비운다(`head` 아래 40px + `rows` 위 24px). 24px일 때는
  소제목과 첫 행이 붙어 보여 머리와 본문이 한 덩어리로 읽혔다
- **밝은 테마만 쓴다.** `noir`·`neon`은 도구에 남아 있지만 콘텐츠에서는 쓰지 않는다.
  쇼츠는 대개 밝은 환경에서 스크롤하며 보고, 어두운 판은 그 자리에서 눈에 띄지만
  읽히지는 않았다
- **고지는 네 줄까지 싣는다** (`QUIET_NOTES`). 자료 범위나 계산 가정을 적는 줄이
  네 번째로 밀리는 주제가 있어서다. 0036은 "평당 5천만원을 넘긴 일곱 곳만
  모았다"는 단서가 네 번째였고, 세 줄로 자르던 동안 카드에도 설명에도 없었다.
  `cards/tools/upload.mjs`도 같은 네 줄을 설명에 넣는다
...
카드를 자기 자신과 대조한다. 망 접근이 필요 없고, 읽어서는 안 보이는 두 가지를
잡는다.

1. **막대가 표시값의 역수인 경우.** 기간을 보여주는 카드에서 막대를 1/기간으로
   그리면 짧게 걸리는 쪽이 긴 막대가 된다. 실제로 0014·0018·0023에서 세 번
   냈던 실수다. 행들의 값 순서와 막대 순서를 쌍으로 비교해 잡는다.
2. **본문 숫자가 행 값과 반올림만큼 어긋나는 경우.** 행은 2,738년인데 고지에는
   2,739년이라고 쓴 식이다.

**도구가 못 잡는 것도 있다.** 본문이 행에 없는 파생값을 말하면(복리와 단리의
차액 같은) 대조할 대상이 없다. 실제로 "1,821만원"이 계산값 1,822만원과 어긋난
것을 도구는 통과시켰고 손으로 잡았다. 그런 주장은 따로 계산해봐야 한다.

지금까지 이 방식으로 잡아 고친 것:

| 카드 | 틀린 내용 | 실제 |
|---|---|---|
| 0011 | 상위 10곳 중 **7곳**이 증권·금융 | 8곳 |
| 0014 | 수익률이 두 배면 기간은 **절반이 아니다** | 50~51%로 거의 절반 |
| 0023 | 월 10만원 상환 시 원금의 **두 배 가까이** | 1.65배 |
| 0028 | 복리·단리 30년 차이 **1,821만원** | 1,822만원 |
| 0024 | 하루 100만원이면 **2,739년** | 2,738년 |
| 0032 | 4대보험으로 매달 **38만원** | 404,891원이라 40만원 |
| 0033 | 집값 말고 **4천만원** | 3,800만원 |
| 0034 | 퇴직금 **4,166만원** | 4,167만원 |
| 0026 | 할부 수수료 **19만원** | 196,415원이라 19만 6천원 |

### 유튜브 제목은 카드와 따로 논다

`youtube.title`은 카드가 읽지 않는 별도 필드다. 그래서 카드의 숫자를 고쳐도
제목에는 옛 숫자가 그대로 남는다. 위 표의 다섯 건이 전부 그렇게 생겼다.
0024와 0028은 카드를 고친 그 턴에 제목을 같이 고치지 않아 남은 것이다.

`factcheck.py`가 두 가지로 잡는다.

1. 반올림 대조에 제목을 넣었다. 카드가 2,738인데 제목이 2,739면 **실패**로 잡는다.
2. 제목의 숫자가 카드 어디에도 그대로 없으면 `~`로 **알리기만** 한다.
   제목은 일부러 반올림하기도 하기 때문이다(13억 7,300만원 → 13억).
   깎아 말한 것인지 반올림인지는 사람만 가를 수 있으니 눈으로 본다.

### 요율 구간은 '이상~미만'이다

0033을 다시 검산하다 9억과 15억 행이 틀린 줄 알았다. 틀린 건 검산 쪽이었다.
중개보수 구간을 `이하`로 잡아서 9억을 0.4% 구간에, 15억을 0.6% 구간에
넣었는데, 실제로는 9억 **이상**이 0.5%, 15억 **이상**이 0.7%다. 경계값이
행에 들어 있으면 부등호 방향 하나로 카드가 통째로 틀린 것처럼 보인다.
검산 코드도 검산 대상이다.
