...
Black Han Sans는 완성형 한글 11,172자 중 2,581자만 담고 있지만, 카드에 쓰는
단위는 모두 들어 있다. 문제는 커버리지가 아니라 밀도다. `대`처럼 획이 많은
글자는 42px 언저리에서 속공간이 메워지고, `neon`의 발광이 얹히면 덩어리로
보인다.

그래서 숫자 뒤 단위는 되도록 값에 붙이지 말고 부제에 `단위: 대`처럼 적는다.
`억`·`만`·`원`·`년`은 밀도가 낮아 값에 붙여도 읽힌다.
